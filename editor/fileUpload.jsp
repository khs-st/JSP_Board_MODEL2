...
<%
	...
	if (item.getSize() > 0) {
		...
		if (!(ext.equals("jpg") || ext.equals("png") || ext.equals("gif") || ext.equals("jpeg")
				|| ext.equals("bmp") || ext.equals("JPG") || ext.equals("PNG") || ext.equals("GIF")
				|| ext.equals("JPEG") || ext.equals("BMP"))) {
			return;
		}

		//파일 기본경로
		String defaultPath = request.getServletContext().getRealPath("/");// todo 서비스 시 폴더 교체 해야함
		//파일 기본경로 _ 상세경로
		String path = defaultPath + "upload/images" + File.separator;
	...
	pw.print("{\"imageurl\" : \"" + url + "upload/images/" + realname + "\",\"filename\":\"" + realname
	+ "\",\"filesize\": 600,\"imagealign\":\"C\"}");
	pw.flush();
	pw.close();
}
%>
